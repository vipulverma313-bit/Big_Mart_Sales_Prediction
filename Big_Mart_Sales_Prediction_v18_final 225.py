...
if __name__ == '__main__':
    ...
    if train_df is not None and test_df is not None:
        ...
        # --- THIS IS YOUR EDITABLE LIST ---
        # Manually select features by uncommenting them.
        # These will be ADDED to the base predictors. SFS is bypassed.
        ...
        # Define base predictors (all columns that are not target, id, or ANY engineered feature)
        all_predictors = [col for col in train_processed.columns if col not in [TARGET_VARIABLE] + ID_COLS]
        base_predictors = [p for p in all_predictors if p not in ALL_POSSIBLE_ENGINEERED_FEATURES]

        # --- NEW: Manually define the final feature set ---
        # We combine the base features with the ones you manually selected.
        optimal_features = base_predictors + manual_engineered_features
        
        print(f"--- Feature Set Definition (Manual) ---")
        print(f"Base predictors ({len(base_predictors)}): {base_predictors}")
        print(f"Manually selected engineered features ({len(manual_engineered_features)}): {manual_engineered_features}")
        discarded_count = len(ALL_POSSIBLE_ENGINEERED_FEATURES) - len(manual_engineered_features)
        print(f"Engineered features being discarded: {discarded_count}")
