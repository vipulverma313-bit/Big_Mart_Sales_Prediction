...
from sklearn.neighbors import KNeighborsClassifier # Added for KNN imputation
...
CURRENT_YEAR = 2025 # Used for calculating Outlet_Age
...
def load_data_and_split(csv_path, train_file, test_file, target_col):
    try:
        ...
        # Note: If running without the files, this will stop execution.
        raise 
...
def preprocess_data(df):
    ...
    # The original code's line for Non-Edible fat content correction is commented out. 
    # Let's re-add it as it's a critical data consistency step.
...
def impute_missing_values(df):
    """
    Imputes missing values using mean for Item_Weight and KNN for Outlet_Size.
    """
    ...
    knn_features = ['Outlet_Location_Type', 'Outlet_Type', 'Outlet_Years', 'Item_MRP']
    df_knn = df[['Outlet_Size'] + knn_features].copy()
    ...
    # 'Outlet_Size' is the target, 'NaN' must be treated as a value for encoding
    df_knn['Outlet_Size_Encoded'] = le_size.fit_transform(df_knn['Outlet_Size'].astype(str))
    
    # Encode predictor features
    df_knn['Outlet_Location_Type_Encoded'] = le_loc.fit_transform(df_knn['Outlet_Location_Type'])
    df_knn['Outlet_Type_Encoded'] = le_type.fit_transform(df_knn['Outlet_Type'])
    ...
    # Final check: impute any remaining NaNs with the mode (just in case KNN failed or data was all NaN)
...
print(f"Combined data shape (Initial): {df_combined.shape}")

# Feature Engineering and Imputation Pipeline
df_combined = preprocess_data(df_combined.copy())
df_combined = impute_missing_values(df_combined.copy()) # KNN Imputation is here
...
print(f"Combined data shape (Post-FE): {df_combined.shape}")
...
rand_search_xgb = RandomizedSearchCV(xgb, xgb_params, cv=kf, scoring=scoring_metric, n_iter=10, verbose=0, n_jobs=-1, random_state=42) # n_iter reduced for faster execution
...
rand_search_lgbm = RandomizedSearchCV(lgbm, lgbm_params, cv=kf, scoring=scoring_metric, n_iter=10, verbose=0, n_jobs=-1, random_state=42) # n_iter reduced for faster execution
...
SUBMISSION_FILE_NAME = 'submission_sales_predictions_Ensemble_AdvancedFE_KNN.csv'
...
try:
    ...
    print("\n------------------------------------------------------------")
    print(f"ENDEAVOR: Target RMSE < 1130 (Goal)")
    ...
    print(f"\n❌ Error saving file (Check S3_BUCKET_PATH and file permissions): {e}")