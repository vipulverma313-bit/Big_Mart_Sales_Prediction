...
# --- NEW: Outlier Capping Function ---

def cap_outliers_iqr(X, X_test, columns, k=1.5):
    """
    Caps outliers in the specified columns of X and X_test using the IQR method 
    (Q1 - k*IQR, Q3 + k*IQR), calculating the limits ONLY on the training data X.
    """
    print(f"Capping outliers using IQR method (k={k}) for: {columns}")
    ...
    for col in columns:
        if col not in X.columns or col not in X_test.columns:
            print(f"Warning: Column '{col}' not found. Skipping capping.")
            continue
            
        # Calculate IQR limits ONLY on the training data (X)
        ...
        # Apply capping to the training set X
        X_capped[col] = np.where(X_capped[col] < lower_bound, lower_bound, X_capped[col])
        X_capped[col] = np.where(X_capped[col] > upper_bound, upper_bound, X_capped[col])
        
        # Apply the SAME capping limits to the test set X_test
...
# 3. Split data back for Target Encoding and Outlier Capping (X is the train part, X_test_final is the test part)
...
# --- NEW STEP: CAP OUTLIERS ON CONTINUOUS FEATURES ---

# Define all continuous features that will be used for modeling later
continuous_features_to_cap = [
    ...
    # Target Encoded Features (often treated as continuous)
    'Item_Identifier_Avg_Sales', 'Outlet_Identifier_Avg_Sales'
]

# Filter features to cap that actually exist in the current DataFrame
continuous_features_to_cap = [c for c in continuous_features_to_cap if c in X.columns]

# Apply the capping function using k=3.0 (more conservative)
X, X_test_final = cap_outliers_iqr(X, X_test_final, continuous_features_to_cap, k=1.5)
...
# The list of continuous features is re-used from the capping step
continuous_features = continuous_features_to_cap
...
def get_best_model(model, params, X, y, cv_folds=10, random_state=0):
...
lasso_params = {
    'model': [Lasso(fit_intercept=True, max_iter=500)],
...
ridge_params = {
    'model': [Ridge(fit_intercept=True, max_iter=500)],
    'model__alpha': [0.1, 0.5, 1.0], 
    'poly__degree': [2]
}

# XGBoost Model 
xgb_model = XGBRegressor(random_state=4, tree_method='hist', n_jobs=-1, eval_metric='rmse', verbosity=0)
...
if best_model_name == 'XGBoost':
    ...
    final_model = XGBRegressor(random_state=4, tree_method='hist', n_jobs=-1, eval_metric='rmse', verbosity=0, **final_model_params)
    final_model.fit(X, y)
