...
RANDOM_STATE = 4
N_FOLDS = 5
N_OPTUNA_TRIALS = 55 # Reduced for a quicker demonstration; 100 is good for thoroughness
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # --- NEW: Item-specific Visibility Feature ---
    print("Creating item-specific visibility ratio...")
    # Calculate mean visibility per item on the training set (after imputation)
    item_avg_visibility = train_processed.groupby('Item_Identifier')['Item_Visibility'].mean().reset_index().rename(columns={'Item_Visibility': 'Item_Mean_Visibility'})
    
    # Map to both train and test sets
    train_processed = train_processed.merge(item_avg_visibility, on='Item_Identifier', how='left')
    test_processed = test_processed.merge(item_avg_visibility, on='Item_Identifier', how='left')
    
    # Fill NaNs for items in test set that were not in train set
    global_mean_visibility = item_avg_visibility['Item_Mean_Visibility'].mean()
    test_processed['Item_Mean_Visibility'].fillna(global_mean_visibility, inplace=True)
    
    # Create the ratio feature
    train_processed['Visibility_to_Item_Mean_Ratio'] = train_processed['Item_Visibility'] / (train_processed['Item_Mean_Visibility'] + 1e-6)
    test_processed['Visibility_to_Item_Mean_Ratio'] = test_processed['Item_Visibility'] / (test_processed['Item_Mean_Visibility'] + 1e-6)
    ...
    # This is your successful feature block
    ...
    train_processed = train_processed.merge(outlet_category_dist[['Outlet_Food_Ratio', 'Outlet_Drinks_Ratio', 'Outlet_NC_Ratio']], on='Outlet_Identifier', how='left')
    test_processed = test_processed.merge(outlet_category_dist[['Outlet_Food_Ratio', 'Outlet_Drinks_Ratio', 'Outlet_NC_Ratio']], on='Outlet_Identifier', how='left')
    test_processed[['Outlet_Food_Ratio', 'Outlet_Drinks_Ratio', 'Outlet_NC_Ratio']] = test_processed[['Outlet_Food_Ratio', 'Outlet_Drinks_Ratio', 'Outlet_NC_Ratio']].fillna(train_processed[['Outlet_Food_Ratio', 'Outlet_Drinks_Ratio', 'Outlet_NC_Ratio']].median())
    ...
    # 2f. Feature Engineering: MRP Bins
    bin_edges = [0, 70, 135, 200, np.inf]
    bin_labels = ['Low_MRP', 'Mid_MRP', 'High_MRP', 'VHigh_MRP']
    train_processed['MRP_Bin'] = pd.cut(train_processed['Item_MRP'], bins=bin_edges, labels=bin_labels, right=False)
    test_processed['MRP_Bin'] = pd.cut(test_processed['Item_MRP'], bins=bin_edges, labels=bin_labels, right=False)
    ...
    # Convert MRP_Bin to string to be used in groupby
    train_processed['MRP_Bin'] = train_processed['MRP_Bin'].astype(str)
    test_processed['MRP_Bin'] = test_processed['MRP_Bin'].astype(str)

    # --- *** NEWLY ADDED CODE BLOCK *** ---
    print("Creating outlet MRP profile features...")
    # Group by Outlet and the new MRP_Bin
    outlet_mrp_dist = train_processed.groupby(['Outlet_Identifier', 'MRP_Bin'])['Item_Identifier'].nunique().unstack(fill_value=0)

    # Calculate total unique items (same as before)
    outlet_mrp_dist['Total_Unique_Items'] = outlet_mrp_dist.sum(axis=1)

    # Create the ratios
    outlet_mrp_dist['Outlet_Low_MRP_Ratio'] = outlet_mrp_dist['Low_MRP'] / outlet_mrp_dist['Total_Unique_Items']
    outlet_mrp_dist['Outlet_Mid_MRP_Ratio'] = outlet_mrp_dist['Mid_MRP'] / outlet_mrp_dist['Total_Unique_Items']
    outlet_mrp_dist['Outlet_High_MRP_Ratio'] = outlet_mrp_dist['High_MRP'] / outlet_mrp_dist['Total_Unique_Items']
    outlet_mrp_dist['Outlet_VHigh_MRP_Ratio'] = outlet_mrp_dist['VHigh_MRP'] / outlet_mrp_dist['Total_Unique_Items']

    # Define the columns to merge
    mrp_ratio_cols = ['Outlet_Low_MRP_Ratio', 'Outlet_Mid_MRP_Ratio', 'Outlet_High_MRP_Ratio', 'Outlet_VHigh_MRP_Ratio']

    # Merge back into train and test
    train_processed = train_processed.merge(outlet_mrp_dist[mrp_ratio_cols], on='Outlet_Identifier', how='left')
    test_processed = test_processed.merge(outlet_mrp_dist[mrp_ratio_cols], on='Outlet_Identifier', how='left')

    # Fill NaNs for test (in case an outlet in test had no items in train, or just as a safeguard)
    test_processed[mrp_ratio_cols] = test_processed[mrp_ratio_cols].fillna(train_processed[mrp_ratio_cols].median())
    # --- *** END OF NEWLY ADDED CODE BLOCK *** ---
    # --- NEW: Outlet Type Profile Ratios ---
    
    print("Creating outlet TYPE profile features...")
    # Group by Outlet_Type and Item_Category
    outlet_type_dist = train_processed.groupby(['Outlet_Type', 'Item_Category'])['Item_Identifier'].nunique().unstack(fill_value=0)
    ...
    # Merge back (note: merging on 'Outlet_Type' now)
    type_ratio_cols = ['Outlet_Type_Food_Ratio', 'Outlet_Type_Drinks_Ratio', 'Outlet_Type_NC_Ratio']
    train_processed = train_processed.merge(outlet_type_dist[type_ratio_cols], on='Outlet_Type', how='left')
    test_processed = test_processed.merge(outlet_type_dist[type_ratio_cols], on='Outlet_Type', how='left')

    # No NaNs expected here, but good practice
    test_processed[type_ratio_cols] = test_processed[type_ratio_cols].fillna(train_processed[type_ratio_cols].median())
    ...
    cols_to_drop = ['Outlet_Establishment_Year', 'Item_First_Appearance_Year', 'Avg_Vis_Type_in_Outlet', 'Item_Category_Raw', 'Outlet_Mean_MRP', 'Item_Mean_Visibility']
    ...
    # Make sure to LabelEncode MRP_Bin as well, since it's now a string
...
if __name__ == '__main__':
  ...
  if train_df is not None and test_df is not None:
    ...
    # --- Master list of ALL engineered features ---
    ALL_POSSIBLE_ENGINEERED_FEATURES = [
      'Item_Popularity', 'Outlet_Mean_Sales','Outlet_Item_Count', 'Item_Age_Proxy', 'Outlet_Food_Ratio', 'Outlet_Drinks_Ratio', 'Outlet_NC_Ratio',
      'Relative_Visibility', 'Visibility_to_Item_Mean_Ratio','Loc_Cat_Sales_Mean', 'Price_per_Unit_Weight', 
      'Item_MRP_to_Outlet_Mean_MRP_Ratio', 'Item_Saleability_Ratio', 
      'Outlet_to_Item_Age_Ratio', 'MRP_Bin', 'Outlet_Low_MRP_Ratio', 'Outlet_Mid_MRP_Ratio', 'Outlet_High_MRP_Ratio', 'Outlet_VHigh_MRP_Ratio',
      'Outlet_Type_Food_Ratio', 'Outlet_Type_Drinks_Ratio', 'Outlet_Type_NC_Ratio'
    ]

    # --- Master list of ALL base features ---
        # These are the original columns (or simple derivations) before advanced engineering
    ALL_POSSIBLE_BASE_FEATURES = [
      ...
      'Outlet_Type',
      'Item_Category', # Created early, but acts as a base feature
      'Outlet_Age'     # Created early, but acts as a base feature
    ]

    # --- EDITABLE LIST 1: Base Features ---
    # Manually select base features by uncommenting them.
    ...
    # --- EDITABLE LIST 2: Engineered Features ---
    # Manually select engineered features by uncommenting them.
    manual_engineered_features = [
      ...
      'Outlet_NC_Ratio',
      # 'Relative_Visibility', 
      # 'Visibility_to_Item_Mean_Ratio',
      # 'Loc_Cat_Sales_Mean', 
      # 'Price_per_Unit_Weight', 
      # 'Item_MRP_to_Outlet_Mean_MRP_Ratio', 
      # 'Item_Saleability_Ratio', 
      # 'Outlet_to_Item_Age_Ratio', 
      # 'Outlet_Low_MRP_Ratio', 'Outlet_Mid_MRP_Ratio', 'Outlet_High_MRP_Ratio', 'Outlet_VHigh_MRP_Ratio',
      # 'Outlet_Type_Food_Ratio', 'Outlet_Type_Drinks_Ratio', 'Outlet_Type_NC_Ratio',
      'MRP_Bin'
    ]
    
    # --- Define the final feature set from your manual lists ---
    # We no longer auto-detect base features. We use your manual list.
    base_predictors = manual_base_features
    optimal_features = base_predictors + manual_engineered_features
    
    print(f"--- Feature Set Definition (Manual) ---")
    print(f"Manually selected base features ({len(base_predictors)}): {base_predictors}")
    print(f"Manually selected engineered features ({len(manual_engineered_features)}): {manual_engineered_features}")
    
    discarded_base_count = len(ALL_POSSIBLE_BASE_FEATURES) - len(base_predictors)
    discarded_eng_count = len(ALL_POSSIBLE_ENGINEERED_FEATURES) - len(manual_engineered_features)
    
    print(f"Base features being discarded: {discarded_base_count}")
    print(f"Engineered features being discarded: {discarded_eng_count}")
    print(f"\nTotal features for training ({len(optimal_features)})")

    
    # --- REMOVED Step 3 (Initial Optuna for SFS) ---
    # This was only needed to speed up SFS.
    
    # --- REMOVED Step 4 (Sequential Forward Selection) ---
    print("\n--- Skipping Step 4: Sequential Forward Selection (SFS) ---")
    print("Using manually defined feature set.")
    
    
    # --- Step 3: Hyperparameter Tuning (on manually selected feature set) ---
    print("\n--- Step 3: Hyperparameter Tuning (on Manually Selected feature set) ---")
    print(f"Optimizing hyperparameters using the {len(optimal_features)} selected features...")
    xgb_final_params_base = {'objective': 'reg:squarederror', 'seed': RANDOM_STATE, 'n_jobs': -1}
    
    # Run Optuna on the manually defined feature subset
    best_xgb_params_final = optimize_hyperparameters(train_processed, optimal_features)
    xgb_final_params_base.update(best_xgb_params_final)


    # --- Step 4: Train Final Model ---
    # Train the single best model on the optimal feature set AND optimal params
    print("\nStep 4: Training final model with selected features and tuned params...")
    final_model, final_rmse = train_xgboost_model(
      train_processed, 
      optimal_features, 
      xgb_final_params_base # Uses the params optimized for the manual set
    )
    ...
    generate_submission(
      final_model, 
      test_processed, 
      original_test_df_ids, 
      optimal_features, 
      "XGBoost_Manual_Features_Optimized"
    )